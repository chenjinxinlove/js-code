...
/**
 * 三级联动地址
 * var City = require("./City-three");
 var City_three =  new City();
 City_three.linkage("main")
 * @constructor
 */
var City_three = function () {
    this._provinceList = [
        {name:'北京', cityList:[
...
City_three.prototype.createEle = function (mainid) {
    if(!document.getElementById(mainid)){
        return ;
    }
    var _mainid = document.getElementById(mainid);
    function _create(create) {
        var _create = "";
        _create = document.createElement("select");
        _create.id=create;
        return _create;
    }
    var _mainid = document.getElementById(mainid);
    var _selelctid = ["province","city","area"];
    for (var i = 0, len = _selelctid.length;i<len;i++) {
        var _child = _create(_selelctid[i]);
        _mainid.appendChild(_child);
    }
}

City_three.prototype.addOptions = function(target,options) {
    for (var i = 0 ,len = options.length;i < len ;i++){
        target.options.add(new Option(options[i],options[i]))
    }
}

City_three.prototype.linkage = function (id) {
    var self = this;
    this.createEle(id);
    var _provinces = [];
        for(var i=0,len=this._provinceList.length;i<len;i++) {
            _provinces.push(this._provinceList[i].name)
        }
    var __province = document.getElementById("province")
    this.addOptions(__province,_provinces);
    __province.onchange = function () {
        var _index = this.selectedIndex,
            _city = document.getElementById('city'),
           _citys = [],
            _cityList =  self._provinceList[_index].cityList;
           _city.options.length = 0;
            document.getElementById("area").options.length = 0;
        for (var i =0,len =_cityList.length;i<len;i++){
            _citys.push(_cityList[i].name);
        }
        self.addOptions(_city,_citys)
    }
    document.getElementById('city').onchange = function () {
        var _index = this.selectedIndex,
            _area = document.getElementById('area'),
            _areas = [],
            _areaList = self._provinceList[__province.selectedIndex].cityList[this.selectedIndex].areaList;
        _area.options.length = 0;
        for(var i = 0 , len = _areaList.length;i<len;i++){
            _areas.push(_areaList[i])
        }
        self.addOptions(_area,_areas)
    }
}

module.exports = City_three;
